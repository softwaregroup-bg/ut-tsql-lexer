{
    function isKeyword(w) {
        return ['ADD','ALL','ALTER','AND','ANY','AS','ASC','AUTHORIZATION','BACKUP','BEGIN','BETWEEN','BREAK','BROWSE','BULK','BY','CASCADE','CASE','CHECK','CHECKPOINT','CLOSE','CLUSTERED','COALESCE','COLLATE','COLUMN',
            'COMMIT','COMPUTE','CONSTRAINT','CONTAINS','CONTAINSTABLE','CONTINUE','CONVERT','CREATE','CROSS','CURRENT','CURRENT_DATE','CURRENT_TIME','CURRENT_TIMESTAMP','CURRENT_USER','CURSOR','DATABASE','DBCC','DEALLOCATE',
            'DECLARE','DEFAULT','DELETE','DENY','DESC','DISK','DISTINCT','DISTRIBUTED','DOUBLE','DROP','DUMP','ELSE','END','ERRLVL','ESCAPE','EXCEPT','EXEC','EXECUTE','EXISTS','EXIT','EXTERNAL','FETCH','FILE','FILLFACTOR',
            'FOR','FOREIGN','FREETEXT','FREETEXTTABLE','FROM','FULL','FUNCTION','GOTO','GRANT','GROUP','HAVING','HOLDLOCK','IDENTITY','IDENTITY_INSERT','IDENTITYCOL','IF','IN','INDEX','INNER','INSERT','INTERSECT','INTO','IS',
            'JOIN','KEY','KILL','LEFT','LIKE','LINENO','LOAD','MERGE','NATIONAL','NEXT','NOCHECK','NONCLUSTERED','NOT','NULL','NULLIF','OF','OFF','OFFSET','OFFSETS','ON', 'ONLY', 'OPEN','OPENDATASOURCE','OPENQUERY','OPENROWSET','OPENXML','OPTION','OR',
            'ORDER','OUTER','OUTPUT','OVER','PERCENT','PIVOT','PLAN','PRECISION','PRIMARY','PRINT','PROC','PROCEDURE','PUBLIC','RAISERROR','READ','READTEXT','RECONFIGURE','REFERENCES','REPLICATION','RESTORE','RESTRICT','RETURN',
            'REVERT','REVOKE','RIGHT','ROLLBACK','ROW', 'ROWCOUNT','ROWGUIDCOL', 'ROWS', 'RULE','SAVE','SCHEMA','SECURITYAUDIT','SELECT','SEMANTICKEYPHRASETABLE','SEMANTICSIMILARITYDETAILSTABLE','SEMANTICSIMILARITYTABLE','SESSION_USER','SET',
            'SETUSER','SHUTDOWN','SOME','STATISTICS','SYSTEM_USER','TABLE','TABLESAMPLE','TEXTSIZE','THEN','THROW','TO','TOP','TRAN','TRANSACTION','TRIGGER','TRUNCATE','TRY_CONVERT','TSEQUAL','UNION','UNIQUE','UNPIVOT','UPDATE',
            'UPDATETEXT','USE','USER','USING','VALUES','VARYING','VIEW','WAITFOR','WHEN','WHERE','WHILE','WITH','WITHIN GROUP','WRITETEXT'
        ].includes(w.toUpperCase());
    }

    function isFunction(w) {
        return ['@@CONNECTIONS', '@@CPU_BUSY', '@@CURSOR_ROWS', '@@DATEFIRST', '@@DBTS', '@@FETCH_STATUS', '@@IDLE', '@@IO_BUSY', '@@LANGID', '@@LANGUAGE', '@@LOCK_TIMEOUT', '@@MAX_CONNECTIONS', '@@MAX_PRECISION',
            '@@NESTLEVEL', '@@OPTIONS', '@@PACK_RECEIVED', '@@PACK_SENT', '@@PACKET_ERRORS', '@@PROCID', '@@REMSERVER', '@@SERVERNAME', '@@SERVICENAME', '@@SPID', '@@TEXTSIZE', '@@TIMETICKS', '@@TOTAL_ERRORS', '@@TOTAL_READ',
            '@@TOTAL_WRITE', '@@VERSION', 'ABS', 'ACOS', 'APP_NAME', 'APPLOCK_MODE', 'APPLOCK_TEST', 'ASCII', 'ASIN', 'ASSEMBLYPROPERTY', 'AT TIME ZONE', 'ATAN', 'ATN2', 'AVG', 'CAST', 'CEILING', 'CERTENCODED', 'CERTPRIVATEKEY',
            'CHAR', 'CHARINDEX', 'CHECKSUM', 'CHECKSUM_AGG', 'COALESCE', 'COL_LENGTH', 'COL_NAME', 'COLUMNPROPERTY', 'CONCAT', 'CONCAT_WS', 'CONVERT', 'COS', 'COT', 'COUNT', 'COUNT_BIG', 'CUME_DIST', 'CURRENT_TIMESTAMP',
            'CURRENT_USER', 'CURSOR_STATUS', 'DATABASE_PRINCIPAL_ID', 'DATABASEPROPERTYEX', 'DATALENGTH', 'DATEADD', 'DATEDIFF', 'DAY', 'DB_ID', 'DB_NAME', 'DEGREES', 'DENSE_RANK', 'DIFFERENCE', 'EXP', 'FILE_ID', 'FILE_IDEX',
            'FILE_NAME', 'FILEGROUP_ID', 'FILEGROUP_NAME', 'FILEGROUPPROPERTY', 'FILEPROPERTY', 'FIRST_VALUE', 'FLOOR', 'FORMAT', 'FORMAT', 'FULLTEXTCATALOGPROPERTY', 'FULLTEXTSERVICEPROPERTY', 'GET_TRANSMISSION_STATUS',
            'GETDATE', 'GETUTCDATE', 'GROUPING', 'GROUPING_ID', 'HAS_PERMS_BY_NAME', 'INDEX_COL', 'INDEXKEY_PROPERTY', 'INDEXPROPERTY', 'IS_MEMBER', 'IS_ROLEMEMBER', 'IS_SRVROLEMEMBER', 'ISDATE', 'ISNULL', 'ISNUMERIC', 'LAG',
            'LAST_VALUE', 'LEAD', 'LEFT', 'LEN', 'LOG', 'LOG10', 'LOWER', 'LTRIM', 'MAX', 'MIN', 'MIN_ACTIVE_ROWVERSION', 'MONTH', 'NCHAR', 'NEWID', 'NEWSEQUENTIALID', 'NTILE', 'NULLIF', 'OBJECT_DEFINITION', 'OBJECT_ID',
            'OBJECT_NAME', 'OBJECT_SCHEMA_NAME', 'OBJECTPROPERTY', 'OBJECTPROPERTYEX', 'ORIGINAL_DB_NAME', 'ORIGINAL_LOGIN', 'PARSENAME', 'PATINDEX', 'PERCENT_RANK', 'PERCENTILE_CONT', 'PERCENTILE_DISC', 'PERMISSIONS', 'POWER',
            'PWDCOMPARE', 'PWDENCRYPT', 'QUOTENAME', 'RADIANS', 'RAND', 'RANK', 'REPLACE', 'REPLICATE', 'REVERSE', 'RIGHT', 'ROUND', 'ROW_NUMBER', 'RTRIM', 'SCHEMA_ID', 'SCHEMA_NAME', 'SCOPE_IDENTITY', 'SERVERPROPERTY',
            'SESSION_USER', 'SIGN', 'SIN', 'SOUNDEX', 'SPACE', 'SQRT', 'SQUARE', 'STATS_DATE', 'STDEV', 'STDEVP', 'STR', 'STRING_AGG', 'STRING_ESCAPE', 'STRING_SPLIT', 'STUFF', 'SUBSTRING', 'SUM', 'SUSER_ID', 'SUSER_NAME',
            'SUSER_SID', 'SUSER_SNAME', 'SYSTEM_USER', 'TAN', 'TEXTPTR', 'TRANSLATE', 'TRIM', 'TYPE_ID', 'TYPE_NAME', 'TYPEPROPERTY', 'UNICODE', 'UPPER', 'USER_ID', 'USER_NAME', 'VAR', 'VARP', 'VERSION', 'YEAR'
        ].includes(w);
    }
